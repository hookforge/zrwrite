...
WORK_DIR="$ROOT_DIR/zig-out/demo/x16_resume"
REMOTE_DIR="/tmp/zrwrite_x16_resume_demo"

mkdir -p "$WORK_DIR"
rm -f \
  "$WORK_DIR/x16_resume" \
  "$WORK_DIR/x16_resume.patched" \
  "$WORK_DIR/x16_resume_payload.o" \
  "$WORK_DIR/x16_resume.zrpb"

echo "[1/6] building host zrwrite cli"
zig build -Doptimize=ReleaseSafe

echo "[2/6] building aarch64 x16 resume smoke target and payload"
zig cc \
...
"$ROOT_DIR/zig-out/bin/zrwrite" bundle \
  --output "$WORK_DIR/x16_resume.zrpb" \
  --payload "$WORK_DIR/x16_resume_payload.o" \
  --hook-kind instrument \
  --target-symbol x16_patchpoint \
  --handler-symbol on_hit \
  --log-message $'zrwrite x16 resume hit\n'

echo "[4/6] applying bundle locally"
"$ROOT_DIR/zig-out/bin/zrwrite" apply \
  --bundle "$WORK_DIR/x16_resume.zrpb" \
  --input "$WORK_DIR/x16_resume" \
  --output "$WORK_DIR/x16_resume.patched"
...
ssh ubuntu@orb "
  set -euo pipefail
  chmod +x '$REMOTE_DIR/x16_resume' '$REMOTE_DIR/x16_resume.patched'
  set +e
  '$REMOTE_DIR/x16_resume' > '$REMOTE_DIR/original.stdout' 2>&1
  original_status=\$?
  '$REMOTE_DIR/x16_resume.patched' > '$REMOTE_DIR/patched.stdout' 2>&1
  patched_status=\$?
  set -e
  ...
  grep -q 'zrwrite x16 resume hit' '$REMOTE_DIR/patched.stdout'
"

echo "x16 resume demo completed successfully"
